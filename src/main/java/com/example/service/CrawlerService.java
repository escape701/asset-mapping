...
public class CrawlerService {
    ...
    private static void appendReportCss(StringBuilder sb) {
        String[] lines = {
                ...
                ".login-group{border:1px solid #e5e7eb;border-radius:12px;overflow:hidden;background:#fff}",
                ".login-group .login-card{border:none;border-radius:0}",
                ".loginCardDupBar{padding:10px 16px;background:#f8fafc;border-top:1px solid #e5e7eb;cursor:pointer;font-size:13px;color:#2477e9;font-weight:500;user-select:none;transition:background .15s}",
                ".loginCardDupBar:hover{background:#eff6ff}",
                ".loginCardDupList{padding:0 16px 12px;background:#f8fafc}",
                ".dupItem{display:flex;align-items:flex-start;gap:12px;padding:8px 12px;border:1px solid #e5e7eb;border-radius:8px;margin-bottom:6px;background:#fff}",
                ".dupThumbWrap{flex-shrink:0;width:120px;height:80px;border-radius:6px;overflow:hidden;border:1px solid #e5e7eb;position:relative;background:#f9fafb}",
                ".dupThumb{width:100%;height:100%;object-fit:cover;cursor:pointer;transition:opacity .15s}.dupThumb:hover{opacity:.8}",
                ".dupThumbPlaceholder{display:flex;align-items:center;justify-content:center;width:100%;height:100%;color:#9ca3af;font-size:11px;background:#f3f4f6}",
                ".dupInfo{flex:1;min-width:0}",
                ".dupUrl{font-size:13px;word-break:break-all}.dupUrl a{color:#2477e9;text-decoration:none}.dupUrl a:hover{text-decoration:underline}",
                ".dupMeta{display:flex;gap:12px;margin-top:4px;font-size:12px;color:#6b7280}",
    ...
    private static void appendReportJs(StringBuilder sb) {
        String[] lines = {
                ...
                "var _SAME_PFX='YES - but same login entry with ';",
                "function isSameEntry(p){if(!p||!p.is_login_page)return false;return String(p.is_login_page).trim().toLowerCase().indexOf(_SAME_PFX.toLowerCase())===0;}",
                "function getSameEntryUrl(p){if(!isSameEntry(p))return null;return String(p.is_login_page).trim().substring(_SAME_PFX.length);}",
                "function groupLogins(logins){var groups=[],cmap={};logins.forEach(function(p){var cu=getSameEntryUrl(p);if(cu){if(!cmap[cu])cmap[cu]=[];cmap[cu].push(p);}else{groups.push({primary:p,dups:[]});}});groups.forEach(function(g){var u=g.primary.url;if(cmap[u]){g.dups=cmap[u];delete cmap[u];}});Object.keys(cmap).forEach(function(u){var arr=cmap[u];groups.push({primary:arr[0],dups:arr.slice(1)});});return groups;}",
                ...
                "function getConf(p){var v=String(p.is_login_page||'').toUpperCase();var verified=v.indexOf('VERIFIED')>=0;if(verified){var l=getLlm(p);var at=l&&l.auth_types?l.auth_types:[];var allUnk=at.length===0||at.every(function(t){return t.toUpperCase()==='UNKNOWN';});return allUnk?'MEDIUM':'HIGH';}if(isSameEntry(p)){var l2=getLlm(p);if(l2){var at2=l2.auth_types||[];var allUnk2=at2.length===0||at2.every(function(t){return t.toUpperCase()==='UNKNOWN';});return allUnk2?'MEDIUM':'HIGH';}return 'MEDIUM';}return 'LOW';}",
                ...
                "  var groups=groupLogins(logins);",
                "  var _co={HIGH:0,MEDIUM:1,LOW:2};groups.sort(function(a,b){var ca=_co[getConf(a.primary)],cb=_co[getConf(b.primary)];return (ca!=null?ca:3)-(cb!=null?cb:3);});",
                "  var totalDups=0;groups.forEach(function(g){totalDups+=g.dups.length;});",
                "  var failed=cr.failed_urls||[];",
                "  var discovered=cr.discovered_urls||disc.urls||[];",
                "  domainCache.push({d:d,subs:subs,vp:vp,logins:logins,groups:groups,totalDups:totalDups,failed:failed,discovered:discovered});",
                "});",
                ...
                "  var primaries=c.groups.map(function(g){return g.primary;});",
                "  var filteredGroups=c.groups.filter(function(g){return matchFilter(g.primary,idx);});",
                "  var h=buildFilterBar(primaries,idx);",
                "  h+='<div class=\"login-grid\">';",
                "  var first=filteredGroups.slice(0,PAGE);",
                "  first.forEach(function(g){h+=renderLoginGroup(g);});",
                "  if(filteredGroups.length>PAGE)h+=loadMoreBtn('logins',idx,first.length,filteredGroups.length);",
                "  h+='</div>';",
                "  if(filteredGroups.length===0)h+='<p style=\"padding:16px;color:#6b7280;text-align:center\">无匹配的登录入口</p>';",
                "  el.innerHTML=h;",
                "}",
                ...
                "function parseStatus(v){if(!v)return{s:'',d:''};var str=String(v).trim();var u=str.toUpperCase();if(u==='YES'||u==='NO')return{s:u,d:''};if(u.indexOf('YES - BUT SAME LOGIN ENTRY WITH')===0)return{s:'YES',d:''};var m=str.match(/^(YES|NO)\\s*[-–]\\s*(.+)$/i);if(m)return{s:m[1].toUpperCase(),d:m[2].trim()};if(u!==''&&u!=='FALSE')return{s:'YES',d:str};return{s:'',d:''};}",
                ...
                "  return h;",
                "}",
                "function renderLoginGroup(g){",
                "  var h=renderLoginCard(g.primary);",
                "  if(g.dups.length){",
                "    var gid='dup-'+Math.random().toString(36).substring(2,8);",
                "    h='<div class=\"login-group\">'+h;",
                "    h+='<div class=\"loginCardDupBar\" onclick=\"var el=document.getElementById(\\''+gid+'\\');if(el){var v=el.style.display===\\'none\\';el.style.display=v?\\'block\\':\\'none\\';this.querySelector(\\'span\\').textContent=v?\\'▼\\':\\'▶\\';}\">';",
                "    h+='<span>▶</span> +'+g.dups.length+' 个相同登录入口</div>';",
                "    h+='<div class=\"loginCardDupList\" id=\"'+gid+'\" style=\"display:none\">';",
                "    g.dups.forEach(function(dp){",
                "      var dpImgs=[];",
                "      if(dp.screenshot_path)dpImgs.push({src:ssrc(dp.screenshot_path),label:'主截图'});",
                "      var dpPopups=dp.popup_login_screenshot_path||[];",
                "      if(dpPopups.length)dpPopups.forEach(function(pp,pi){dpImgs.push({src:ssrc(pp),label:'弹窗截图'+(dpPopups.length>1?(' '+(pi+1)):'')});});",
                "      var dpImgJson=dpImgs.length?JSON.stringify(dpImgs).replace(/\"/g,'&quot;'):'';",
                "      h+='<div class=\"dupItem\" data-title=\"'+esc(dp.title||'登录页面截图')+'\" data-url=\"'+esc(dp.url||'')+'\"'+(dpImgJson?' data-imgs=\"'+dpImgJson+'\"':'')+'>';",
                "      h+='<div class=\"dupThumbWrap\">';",
                "      if(dp.screenshot_path){h+='<img class=\"dupThumb\" src=\"'+ssrc(dp.screenshot_path)+'\" alt=\"'+esc(dp.title||'')+'\" onerror=\"this.style.display=\\'none\\';this.nextElementSibling.style.display=\\'flex\\'\">';h+='<div class=\"dupThumbPlaceholder\" style=\"display:none\">暂无截图</div>';}",
                "      else{h+='<div class=\"dupThumbPlaceholder\">暂无截图</div>';}",
                "      h+='</div>';",
                "      h+='<div class=\"dupInfo\">';",
                "      h+='<div class=\"dupUrl\"><a href=\"'+esc(dp.url||'')+'\" target=\"_blank\">'+esc(dp.url||'')+'</a></div>';",
                "      h+='<div class=\"dupMeta\">';",
                "      if(dp.title)h+='<span>'+esc(dp.title)+'</span>';",
                "      if(dp.status_code)h+='<span>状态: '+dp.status_code+'</span>';",
                "      h+='</div></div></div>';",
                "    });",
                "    h+='</div></div>';",
                "  }",
                ...
                "  else if(type==='logins')src=c.groups.filter(function(g){return matchFilter(g.primary,idx);});",
                ...
                "  else if(type==='logins')next.forEach(function(g){h+=renderLoginGroup(g);});",
                ...
                "    return;",
                "  }",
                "  var dupThumb=e.target.closest('.dupThumb');",
                "  if(dupThumb){",
                "    var di=dupThumb.closest('.dupItem');if(!di||!di.dataset.imgs)return;",
                "    var dImgs=JSON.parse(di.dataset.imgs||'[]');if(!dImgs.length)return;",
                "    openSSModal(di.dataset.title||'',di.dataset.url||'',dImgs,0);return;",
                "  }",
